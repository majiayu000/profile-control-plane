...
function flagshipSignals(config: ProfileConfig, palette: Palette): string {
  ...
  const nodes = config.flagships
    ...
    .map((project, index) => {
      ...
      return `<g class="boot" style="animation-delay:${(index * 0.18).toFixed(2)}s"><circle cx="${position.x}" cy="${position.y}" r="35" fill="${palette.panel}" stroke="${color}" stroke-width="2"/><circle class="pulse" cx="${position.x}" cy="${position.y}" r="7" fill="${color}" filter="url(#industrial-glow)"/>
...
export class SignalGridRenderer implements ThemeRenderer {
  renderHero(config: ProfileConfig, mode: ColorMode): string {
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="360" viewBox="0 0 1200 360" role="img" aria-label="${escapeXml(config.identity.name)} signal grid" data-mode="${mode}">
      ...
      <g class="boot"><path d="M36 70H648V282H36Z" fill="${palette.panel}" fill-opacity=".34" stroke="${palette.line}"/><path d="M36 70h84M36 70v52M648 282h-84M648 282v-52" stroke="${palette.primary}" stroke-width="2"/>
